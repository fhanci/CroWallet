...
const GOLD_TYPES = {
  GRAM_GOLD: { label: "Gram Altın", symbol: "gr" },
  CEYREK_GOLD: { label: "Çeyrek Altın", symbol: "adet" },
  YARIM_GOLD: { label: "Yarım Altın", symbol: "adet" },
  TAM_GOLD: { label: "Tam Altın", symbol: "adet" },
  CUMHURIYET_GOLD: { label: "Cumhuriyet Altını", symbol: "adet" },
};

const MyGoldAccountsPage = () => {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      // Filter accounts that have GOLD holdings
      const goldAccounts = response.data.filter((acc) =>
        acc.holdings?.some((h) => h.assetType === "GOLD")
      );

      setAccounts(goldAccounts);
      setFilteredAccounts(goldAccounts);
    } catch (error) {
      ...
      filtered = filtered.filter(
        (acc) =>
          ...
          acc.holdings?.some((h) =>
            ...
            getGoldTypeLabel(h.assetSymbol).toLowerCase().includes(searchQuery.toLowerCase())
          )
      );
    }
  ...
  const getGoldTypeLabel = (symbol) => {
    return GOLD_TYPES[symbol]?.label || symbol;
  };

  const getGoldTypeUnit = (symbol) => {
    return GOLD_TYPES[symbol]?.symbol || "adet";
  };
  ...
  const calculateTotals = () => {
    ...
    accounts.forEach((acc) => {
      acc.holdings?.filter((h) => h.assetType === "GOLD").forEach((holding) => {
  ...
  const getGoldHoldings = (account) => {
    return account.holdings?.filter((h) => h.assetType === "GOLD") || [];
  };

  return (
    <Container maxWidth="lg" sx={{ mt: 3, mb: 4 }}>
      <Typography variant="h4" sx={{ fontWeight: 600, mb: 3, display: "flex", alignItems: "center", gap: 1 }}>
        <ViewInArIcon sx={{ fontSize: 36, color: "#FFD700" }} />
        Altın Hesaplarım
      </Typography>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 4, flexWrap: "wrap" }}>
        ...
        >
          ...
          <Typography variant="h5" sx={{ fontWeight: 700, color: isDarkMode ? "#fff" : "inherit" }}>
            {totals.totalGolds} / {totals.accountCount}
          </Typography>
        </Card>
      </Box>
      ...
      <Box sx={{ mb: 3 }}>
        <TextField
          placeholder="Hesap veya altın türü ara..."
          size="small"
          value={searchQuery}
      ...
      {loading ? (
        <Box sx={{ width: "100%" }}>
          <LinearProgress sx={{ bgcolor: "rgba(255, 215, 0, 0.3)", "& .MuiLinearProgress-bar": { bgcolor: "#FFD700" } }} />
        </Box>
      ) : filteredAccounts.length === 0 ? (
        <Alert severity="info" sx={{ borderRadius: 2 }}>
          {accounts.length === 0
            ? "Henüz altın hesabı bulunmamaktadır."
            : "Aramaya uygun hesap bulunamadı."}
        </Alert>
      ) : (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {filteredAccounts.map((account) => {
            const goldHoldings = getGoldHoldings(account);
            const accountValue = goldHoldings.reduce(
              (sum, h) => sum + (h.quantity || 0) * (h.currentPrice || 0),
              0
            );
            const accountCost = goldHoldings.reduce(
                ...
                <CardContent sx={{ p: 2.5 }}>
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      <ViewInArIcon sx={{ fontSize: 40, color: "#FFD700" }} />
                      <Box>
                        <Typography variant="h6" sx={{ fontWeight: 600 }}>
                          {account.accountName}
                        </Typography>
                        <Box sx={{ display: "flex", gap: 1, mt: 0.5, flexWrap: "wrap" }}>
                          {goldHoldings.slice(0, 3).map((holding) => (
                            <Chip
                              key={holding.id}
                              label={getGoldTypeLabel(holding.assetSymbol)}
                              size="small"
                              sx={{
                          ...
                          {goldHoldings.length > 3 && (
                            <Chip
                              label={`+${goldHoldings.length - 3}`}
                              size="small"
                              variant="outlined"
                            />
                          )}
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      <Box sx={{ textAlign: "right", mr: 1 }}>
                        <Typography variant="h5" sx={{ fontWeight: 700, color: "#B8860B" }}>
                  ...
                  <Box sx={{ mt: 2, display: "flex", gap: 2, flexWrap: "wrap" }}>
                    {goldHoldings.slice(0, 4).map((holding) => {
                      const value = (holding.quantity || 0) * (holding.currentPrice || 0);
                      ...
                      return (
                        <Box
                          ...
                          sx={{
                            p: 1.5,
                            bgcolor: profit >= 0 ? "rgba(255, 215, 0, 0.1)" : "rgba(244, 67, 54, 0.08)",
                            borderRadius: 2,
                            minWidth: 150,
                        ...
                        >
                          <Typography variant="body2" sx={{ fontWeight: 600, color: "#B8860B" }}>
                            {getGoldTypeLabel(holding.assetSymbol)}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            {holding.quantity} {getGoldTypeUnit(holding.assetSymbol)} • {formatCurrency(holding.currentPrice)}
                          </Typography>
                        </Box>
                      );
                    })}
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} maxWidth="xs" fullWidth disableScrollLock>
        ...
        <DialogContent>
          <Typography sx={{ mb: 2 }}>
            <strong>{deletingAccount?.accountName}</strong> hesabını ve tüm altın varlıklarını silmek istediğinizden emin misiniz?
          </Typography>
          <TextField
...
export default MyGoldAccountsPage;
